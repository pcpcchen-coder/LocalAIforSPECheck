...
v0.2.1 將動態執行資訊與包含文件快照的 Run 資料分開保存。`run_progress` 紀錄目前階段、規範／區塊／產品視窗位置、請求計數、目前請求開始時間、最近模型回覆、近期事件及停止等待狀態。執行階段由實際工作者與模型呼叫邊界更新，不使用動畫或前端計時模擬工作進度。

前端以 `GET /api/runs/{id}/progress` 取得小型狀態資料；輪詢不重傳全部來源文件、證據及覆核歷史。需要結果明細時仍使用完整 Run API。狀態查詢應保持輕量，不能為每次頁面更新重新掃描所有文件。

| 指標 | 意義與界線 |
|---|---|
| 已完成區塊／百分比 | 只計入已完整處理並儲存的規範區塊；目前視窗尚未完成時不增加 |
| 產品視窗／模型請求 | 反映實際送出的工作與呼叫完成狀態；請求完成不表示回覆一定有效 |
| 目前請求與批次時間 | 由保存的時間資訊顯示經過時間，不推估 token 速度或剩餘完成時間 |
| 最近模型回覆 | 模型 API 回覆的時間依據；格式驗證與引述驗證仍是後續獨立步驟 |
| 狀態連線更新 | 僅表示瀏覽器仍可查到本機應用程式，不能據此證明模型程序仍在運算 |
| 近期事件 | 用來解釋目前等待原因與處理變化；不替代正式結果及人工覆核歷史 |

取消時顯示已請求停止、等待目前模型呼叫結束；避免把取消按鈕送出成功解讀為模型已立即中止。若輪詢失敗或狀態過久沒有更新，前端標示連線或狀態過期警告，保留最後收到的內容供判讀，不將計時增加呈現為正常工作的證據。

舊資料庫可沿用。缺少 `run_progress` 的歷史批次採相容顯示，不補造舊事件或模型回覆時間；原結果、人工覆核與匯出不受影響。進度資料不包含模型金鑰，也不以每次輪詢傳送文件全文。
...
| `GET /api/runs/{id}/progress` | 輕量執行現況與完成計數，不包含文件全文、全部結果及覆核歷史 |
