...
查證日期：2026-09-21。本文件記錄 Windows x64 Portable 使用的固定上游元件，供重建、來源追溯與內部審查。最終 ZIP 的實際內容與雜湊，以該次發行隨附的 manifest 為準。
...
| 內建文字模型 | LM Studio Community `Qwen3.5-2B-Q6_K.gguf` | 1,556,390,368 bytes |
...
模型來源是 [LM Studio Community 量化版本](https://huggingface.co/lmstudio-community/Qwen3.5-2B-GGUF/tree/bb84e11355a036e28f080c7793fa6d22b7c4e344)，原始模型由 [Qwen](https://huggingface.co/Qwen/Qwen3.5-2B) 發布。Portable 沒有重包 LM Studio 桌面應用程式，也不需要使用者安裝 LM Studio。內建 GGUF 僅用於文字推論；未附影像投影模型，不代表包含 OCR。
...
Python 與 llama.cpp ZIP 的 SHA-256 已實際下載核對；模型 SHA-256 與大小來自 Hugging Face 的 LFS metadata，建置時需對下載完成的模型再次核對。llama.cpp CPU release asset ID 為 `563672535`。[Python 發行資訊](https://www.python.org/downloads/release/python-31315/)、[llama.cpp 固定版本](https://github.com/ggml-org/llama.cpp/releases/tag/b10964)

GitHub Releases 的單一 asset 必須小於 2 GiB，因此內建模型採 2B Q6_K，建置流程也應檢查最終 ZIP 大小。模型檔與依賴應放在 Release ZIP，不提交至 Git 原始碼歷史。[GitHub 官方限制](https://docs.github.com/en/repositories/releasing-projects-on-github/about-releases#storage-and-bandwidth-quotas)
...
| `--model <本地 GGUF>` | 明確載入已隨 ZIP 交付的模型；離線使用不應依賴 `--hf-repo` 自動下載。 |
...
內建 Qwen3.5-2B Q6_K 是讓使用者下載單一 ZIP 後即可啟動的入門模型。它能用於初步比對與流程驗證；條文例外、測試條件、否定句、表格跨頁、數值與單位推理，仍可能出現漏判或誤判。
